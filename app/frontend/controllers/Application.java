...
public class Application extends Controller {
	...
	public static Result logout() {
        ...
        return redirect(routes.ClimateServiceController.newHome(null, null, null));
    }
	...
	public static Result authenticate() {
		...
		String emailFilled = loginForm.field("email").value();
		String passwordFilled = loginForm.field("password").value();
		String validationStirng = validate(emailFilled,passwordFilled);
		
	    if (loginForm.hasErrors()||!validationStirng.equals("valid")) {
	    	System.out.println("*********2");
	        return badRequest(login.render(loginForm));
	        // return badRequest("invalid login");
	    } else {
	        session().clear();
	        session("email", emailFilled);
	        System.out.println("reached nearly final");
	        return redirect(routes.ClimateServiceController.newHome("", "", ""));
	    }
	}
    ...
    public static Result createNewUser(){
    	...
    	try{
			...
			System.out.println("registering" + userName+nu.get().getEmail()+nu.get().getPassword());
			JsonNode response = APICall.postAPI(Constants.URL_HOST + Constants.CMU_BACKEND_PORT 
    ...
    public static Result isEmailExisted() {
		...
		try {
			...
			System.out.println("email exist address is     "+Constants.URL_HOST + Constants.CMU_BACKEND_PORT 
