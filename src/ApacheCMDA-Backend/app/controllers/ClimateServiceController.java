	...
	public Result addClimateService() {
		...
		try {
			...
			ClimateService climateService = new ClimateService(rootServiceId,
					user, name, purpose, url, scenario, createTime, versionNo, (double)0, 0);
...
System.out.println("Backend Calling getAllClimateServices");
...
//                .findByOrderByCreateTimeDesc();
    ...
    public Result getAllServiceEntries(String format) {
    	...
    	//design my ourselves - to find climate service by search key!!!
	public Result getClimateServiceByKeyWord(String key, String format) {

		System.out.println("start to search climate service by key words");
		Iterable<ClimateService> climateServices = climateServiceRepository.getClimateServiceByKeyWord("%"+key+"%");
        if (climateServices == null) {
            System.out.println("No climate service found");
        }
System.out.println("get service successfully");
List<ClimateService> resultclimatelist = new ArrayList<ClimateService>();
        List<String> namelist = new ArrayList<String>();
        for(ClimateService cs : climateServices) {
        	System.out.println("hahaha");
        	if(!namelist.contains(cs.getName())) {
        		if(namelist.size() == 3) break;
        		namelist.add(cs.getName());
        	}
        	resultclimatelist.add(cs);	
        }

        String result = new String();
        if (format.equals("json")) {
            result = new Gson().toJson(resultclimatelist);
        }
